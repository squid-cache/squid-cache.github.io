...
A file number of ''FFFFFFFF'' indicates "memory only" objects. Any
...
[5] '''hash''' The hash value used to index the object in the cache.
Squid currently uses MD5 for the hash value.
...
[10] '''type''' The HTTP ''Content-Type'' major value, or "unknown"
...
The '''datehdr''', '''lastmod''', and '''expires''' values are
all expressed in UTC seconds. The
...
The
[http://www.w3.org/Daemon/User/Config/Logging.html#common-logfile-format Common Logfile Format]
...
We recommend that you use Squid's native log format due to its
...
[1] '''time''' A Unix timestamp as UTC seconds with a millisecond resolution. You
...
[2] '''duration''' The elapsed time considers how many milliseconds
the transaction busied the cache. It differs in interpretation
between TCP and UDP:
...
[3] '''client address''' The IP address of the requesting instance, the client IP address. The
...
how it was satisfied, or in what way it failed. Please refer to
[#squid_result_codes Squid result codes] for valid symbolic result codes.

Several codes from older versions are no longer available, were
renamed, or split. Especially the ''ERR_'' codes do not seem to
appear in the log file any more. Also refer to
[#squid_result_codes Squid result codes] for details
on the codes no longer available in Squid-2.

The NOVM versions and Squid-2 also rely on the Unix buffer cache, thus
you will see less ''TCP_MEM_HIT''s than with a Squid-1.
...
extensions. Squid uses a subset of the RFC defined error codes for HTTP.
Refer to section [#http_status_codes status codes] for details of the
status codes ecognized by a Squid-2.

[5] '''bytes''' The size is the amount of data delivered to the client. Mind that this does
...
[6] '''request method''' The request method to obtain an object. Please refer to section
...
[7] '''URL''' This column contains the URL requested. Please note that the log file
may contain whitespaces for the URI. The default configuration for
''uri_whitespace'' denies whitespaces, though.

[8] '''rfc931''' The eigth column may contain the ident lookups for the requesting
...
'''TCP_REFRESH_HIT''' The requested object was cached but ''STALE''.
The IMS query for the object resulted in "304 not modified".

'''TCP_REF_FAIL_HIT''' The requested object was cached but ''STALE''.
The IMS query failed and the stale object was delivered.
...
'''TCP_NEGATIVE_HIT''' Request for a negatively cached object,
...
'''TCP_MEM_HIT''' A valid copy of the requested object was in the
cache ''and'' it was in memory, thus avoiding disk accesses.

'''TCP_DENIED''' Access was denied for this request.

'''TCP_OFFLINE_HIT''' The requested object was retrieved from the
...
'''UDP_HIT_OBJ''' Refers to an old version that would send
cache hits in ICP replies.  No longer implemented.
...
[ftp://ftp.isi.edu/in-notes/rfc2616.txt RFC 2616] and verified for Squid. Squid-2 uses almost all
...
[ftp://ftp.isi.edu/in-notes/rfc2518.txt RFC 2518] (WebDAV).
...
also recognize
[ftp://ftp.isi.edu/in-notes/rfc2518.txt RFC 2518] "HTTP Extensions for Distributed Authoring --
WEBDAV" extensions.
...
'''FIRST_UP_PARENT''' The object was fetched from the first parent
...
'''FIRST_PARENT_MISS''' The object was fetched from the parent with
the fastest (possibly weighted) round trip time.
...
firewall configuration, see also ''never_direct'' and related
material, and no parents were available.

'''SOURCE_FASTEST''' The origin site was chosen, because the source
ping arrived fastest.

'''ROUNDROBIN_PARENT''' No ICP replies were received from any parent.
The parent was chosen, because it was marked for round robin in the
config file and had the lowest usage count.
...
a binary file that includes MD5 checksums, and ''Store''''''Entry'' fields.
Please see the Programmers' Guide for information on the contents and format of that file.
...
developed some
[http://www.uninett.no/prosjekt/desire/arneberg/statistics.html some basic rules]
