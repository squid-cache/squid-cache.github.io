...
L2 forwarding is best suited for when the proxy is directly connected to the router. Since Layer-2 is a level below TCP/IP it can be treated as equivalent to ''Policy Routing'' at the IP layer and requires only routing configuration on the receiving proxy machine. Also note, L2 forwarding most often hardware accelerated and has no additional overhead (because uses L2 header re-writes without increasing packet), so it has best performance in most cases.
