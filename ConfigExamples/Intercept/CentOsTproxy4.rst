## page was renamed from ConfigExamples/TPROXYPatchingCentOS
...
These steps are for setting [[Squid-3.1]] with [[Features/Tproxy4|TPROXYv4]], IP spoofing and Cisco WCCP. This is not a bridging setup. It is also important to note that these steps are specific to the x86_64 version of CentOS 5.3, although very minor changes would make this solution work on any version of CentOS 5 (I try to make notes where there are differences.)
...
=== Preparation ===
 ...
 1. Once the install completes and you have booted into the OS, run: '''yum update''' (and apply all updates.)
 1. Once the yum command completes, '''reboot'''
 1. Download iptables-1.4.3.2 from netfilter.org.
 1. Download kernel 2.6.30 from kernel.org.
 1. Download (squid 3.1.0.8 or higher) [[http://www.squid-cache.org/Versions/v3/3.1/|Squid v3.1 source code]]
 1. decompress the kernel source to /usr/src/linux-2.6.30. (I have read all over the place that kernel source should not reside in /usr/src, I use the path here as an example, you can put the source anywhere.) Make a symbolic link to /usr/src/linux when done:
 ...
 * Next check iptables versioning to make sure it installed properly in the right path:
  a. "iptables -v" should show: b. iptables v1.4.0: no command specified Try `iptables -h' or 'iptables --help' for more information.
 ...
 * Is the following iptables commands to enable TPROXY functionality in the running iptables instance:
