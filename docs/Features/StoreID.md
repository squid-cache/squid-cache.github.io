...
- **Status**: completed.
- **Version**: 3.4
- **Developer**: [Eliezer Croitoru](/EliezerCroitoru)
- **More**:
- **Sponsored by**:
    [Eliezer Croitoru](/EliezerCroitoru) and
...
- Eliezer Croitoru has designed several Ruby helpers, including
        [example helper](/Features/StoreID/Helper)
- **storeid_file_rewrite** by Alan Mizrahi is a simple helper which
    is packaged with
    [Squid-3.4](/Releases/Squid-3.4).
    It can be used to load a [database of patterns](http://wiki.squid-cache.org/Features/StoreID/DB) without
...
provides de-duplication for content such as the
[SourceForge](https://sourceforge.net/)
CDN or Linux distributions repository mirrors. Contributions are
welcome.
...
unique download url/token. If we will inspect the full request and
...
> :information_source:
    from [Squid-3.5](/Releases/Squid-3.5)
...
There is a newer [StoreID helper](/Features/StoreID/Helper)
...
:   One or more key=value pairs. Only "myip" and "myport" pairs
    ...
    by [Squid-3.4](/Releases/Squid-3.4)
    and older:
...
:   One or more key=value pairs. The key names reserved on this
    ...
    [Store URL-rewrite](/Features/StoreUrlRewrite)
    feature helpers written for
    [Squid-2.7](/Releases/Squid-2.7).
    However this syntax is deprecated and such helpers should be
