...
[[http://www.freebsd.org/cgi/ports.cgi?query=^squid&stype=name|squid-4.15]] in the ports collection.
...
The [[SquidFaq/CompilingSquid|general build instructions]] should be all you need.
 ...
 * {{{ /usr/local/etc/squid }}} - configurations (squid.conf, mime.conf, error pages)
