...
The data member is used to accumulate data across squid instances. Execution flow is:
 ...
 1. (without going in too much detail) this Action's {{{run()}}} method calls each worker's {{{collect()}}} method
 1. collect() is supposed to fill in the data member of the !MyModuleMgrAction with whatever data is relevant
 1. data is marshaled back to the coordinator process via {{{pack()}}} and {{{unpack()}}}
