...
IT = 10.4.0.0/16, Research & Development = 10.5.0.0/16, Administration = 10.6.0.0/20
...
IT -> 10.9.1.4, Research & Development -> 10.9.1.5, Administration -> 10.9.1.6
...
For instance, Google search results can be set to the strictest [[http://support.google.com/websearch/bin/answer.py?hl=en&answer=2521692|SafeSearch]] level by appending `&safe=active` to the search URL. By rewriting as such the URLs of all Google search queries, we ensure that all LAN users get only safe content.

(Note that Google is [[http://support.google.com/websearch/bin/answer.py?hl=en&answer=173733|gradually switching to HTTPS for all searches]]. As Squid only handles HTTP traffic, this won't work anymore.  However, you get the idea.)

[[http://squirm.foote.com.au/|Download the latest version of Squirm (squirm-1.0betaB)]], untar it, then issue the following commands:
...
[[http://sarg.sourceforge.net/|SARG (Squid Analysis Report Generator)]] is a nice tool that generates stats about client IPs, visited websites, amount of downloaded data, and so on.
