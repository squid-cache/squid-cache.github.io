...
Some resolvers have been identified which support EDNS on IPv6 address lookups (AAAA)) but which fail or reject requests with EDNS on IPv4 address lookups (A).
 ...
 {i} To resolve this Squid is currently hard-coded not to send EDNS hints on the IPv4 A lookups.
...
Due to design issues within Squid we cannot (yet) make use of these hints to automate the packet sizing. Instead the configuration option SquidConf:dns_packet_max is needed to set the advertised packet size or disable EDNS entirely.
